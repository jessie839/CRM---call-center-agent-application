...
import LiveAISuggestions from '../components/Dashboard/LiveAISuggestions';
import '../styles/CallLogsTab.css';
import { PhoneIncomingIcon, PhoneMissedIcon, PhoneOutgoingIcon, Users, ArrowRightLeft, Phone, PhoneForwarded, Keyboard } from 'lucide-react';
...
export default function CallLogsTab({ setActiveLead, setActiveTab }) {
  ...
  const SCHEDULING_OUTCOMES = ['Call back', 'Follow-up Required', 'Meeting'];
  ...
  const handleDispositionSubmit = () => {
    const isScheduled = SCHEDULING_OUTCOMES.includes(dispositionOutcome);
    setShowDisposition(false);
    setCallState({ ...callState, isActive: false });
    setActiveCallAction(null);
    setConferenceParticipants([]);
    setDispositionNote('');
    setDispositionDateTime('');
    setDispositionOutcome('Resolved');
    if (isScheduled) {
      const label = dispositionOutcome === 'Meeting' ? 'Meeting' : 'Callback';
      showSnackbar(`✓ ${label} has been initiated — we will notify you at the scheduled time.`);
    }
  };
  ...
  return (
    <BaseWrapper tabProps={{ tabs: [], headerText: "Call Logs" }}>
      <div className="dash-content-area fade-in cl-container">
          ...
          <div className="ca-modal-overlay fade-in">
            <div className="ca-modal-card fade-in">
              ...
              <div className="ca-modal-content">
                ...
                {SCHEDULING_OUTCOMES.includes(dispositionOutcome) && (
                  <div className="ca-modal-field ca-datetime-field fade-in">
                    <label>
                      ...
                      {dispositionOutcome === 'Meeting' ? 'Meeting Date & Time' : 'Callback Date & Time'}
                    </label>
                    <input
                      type="datetime-local"
                      value={dispositionDateTime}
                      onChange={e => setDispositionDateTime(e.target.value)}
                      className="ca-datetime-input"
                    />
                  </div>
                )}
        ...
        {snackbar && (
          <div className="ca-snackbar fade-in">
            ...
            <span className="ca-snackbar-text">{snackbar.message}</span>
          </div>
        )}
      </div>
    </BaseWrapper>
