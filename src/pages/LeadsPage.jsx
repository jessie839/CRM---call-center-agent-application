...
import { Phone, Filter, Pencil, Pause, Play } from 'lucide-react';
...
function AddLeadScreen({ onSave, onCancel }) {
  ...
  // Personal
  middleName: "", dateOfBirth: "", gender: "", nationality: "", language: "",
  // Address
  city: "", state: "", zipCode: "", country: "",
  // Professional
  company: "", jobTitle: "", industry: "", website: "",
  // Lead Meta
  source: "", priority: "", assignedTo: "", campaign: "",
  // Social
  linkedin: "", whatsapp: "", notes: "",
});
...
const SECTIONS = [
  {
    ...
    fields: [
      ...
      { key: "middleName", label: "Middle Name",        type: "text",   placeholder: "Middle Name" },
      ...
      { key: "nationality",label: "Nationality",        type: "text",   placeholder: "Nationality" },
      { key: "language",   label: "Preferred Language", type: "select", options: ["English", "Urdu", "Arabic", "French", "Spanish"] },
    ]
  },
  {
    ...
    fields: [
      ...
      { key: "zipCode",  label: "ZIP / Postal",   type: "text", placeholder: "ZIP Code" },
      { key: "country",  label: "Country",        type: "select", options: ["Pakistan", "United States", "United Kingdom", "UAE", "Saudi Arabia", "Canada", "Australia"] },
    ]
  },
  {
    title: "Professional Information",
    fields: [
      { key: "company",   label: "Company Name",  type: "text",   placeholder: "Company" },
      { key: "jobTitle",  label: "Job Title",     type: "text",   placeholder: "Job Title" },
      { key: "industry",  label: "Industry",      type: "select", options: ["Technology", "Finance", "Healthcare", "Real Estate", "Retail", "Education", "Other"] },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "disposition",label: "Lead Status",    type: "select", options: ALL_DISPOSITIONS },
      { key: "source",     label: "Lead Source",    type: "select", options: ["Web", "Referral", "Cold Call", "Social Media", "Event", "Email Campaign"] },
      { key: "priority",   label: "Priority",       type: "select", options: ["High", "Medium", "Low"] },
      { key: "assignedTo", label: "Assigned To",    type: "select", options: ["Agent A", "Agent B", "Agent C", "Agent D"] },
      { key: "campaign",   label: "Campaign",       type: "select", options: ["Q1 Campaign", "Q2 Campaign", "Summer Sale", "Retention Drive"] },
      { key: "tags",       label: "Tags",           type: "select", options: ["Hot", "Warm", "Cold", "VIP"] },
      { key: "contacts",   label: "Associated Contacts", type: "select", options: ["Contact A", "Contact B", "Contact C"] },
    ]
  },
  {
    title: "Social & Notes",
    fields: [
      { key: "linkedin",  label: "LinkedIn",  type: "url",      placeholder: "LinkedIn profile URL" },
      { key: "whatsapp",  label: "WhatsApp",  type: "tel",      placeholder: "WhatsApp number" },
      { key: "notes",     label: "Notes",     type: "textarea", placeholder: "Additional notes about this lead..." },
    ]
  },
];
  ...
  return (
    <div className="lm-add-screen">
      <div className="lm-add-screen-header">
        <div className="lm-add-screen-header-left">
          <button className="lm-btn-back" onClick={onCancel} title="Go Back">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <line x1="19" y1="12" x2="5" y2="12"></line>
              <polyline points="12 19 5 12 12 5"></polyline>
            </svg>
          </button>
          <span className="lm-add-screen-title">Add New Lead</span>
        </div>
      ...
      <div className="lm-add-screen-body">
        {SECTIONS.map(section => (
          <div className="lm-add-card" key={section.title}>
            <div className="lm-add-card-section-header">
              <div className="lm-add-card-section-dot" />
              <span className="lm-add-card-section-title">{section.title}</span>
            </div>
            ...
            <div className="lm-add-fields-grid">
              {section.fields.map((f, i) => (
                <div
                  key={f.key}
                  className={`lm-add-field${
                    f.type === "textarea" ? " lm-add-field--full" : i % 2 === 0 ? " lm-add-field--left" : ""
                  }`}
                >
                  ...
                  {f.type === "select" ? (
                    ...
                    <textarea
                      className="lm-add-field-textarea"
                      value={form[f.key]}
                      onChange={e => set(f.key, e.target.value)}
...
export default function LeadManagement() {
  ...
  return (
    <BaseWrapper tabProps={{ tabs: [], headerText: "Leads" }}>
     ...
     <LeadDetailPage
       ...
       onSave={(updated) => {
         ...
         setSelectedLeadDetail(updated);   // keep detail in sync after save
       }}
     />
      ):showAdd ? (
  ...
  <>       {/* Search input field */}
          <div className="lm-topbar-actions">
            ...
            <div className="lm-topbar-right">
                ...
                Header Fields
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button
                      ...
                      title={selected.size === 0 ? "Select leads to start auto dial" : "Start Auto Dial"}
                    >
              ...
              <button className="lm-btn-add-lead" onClick={() => setShowAdd(true)}>
                <span className="lm-btn-add-lead-plus">+</span> Add Lead
              </button>
            </div>
          </div>
          ...
          <div className="lm-table-container">
            <div className="lm-table-card">
              ...
              {/* Empty state */}
              {paginated.length === 0 && (
                <div className="lm-empty">No leads match your search.</div>
              )}
      ...
      if (f.key === "action") return (
        <span key="action" className="lm-cell-action" style={{ display: 'flex', justifyContent: 'center', gap: '8px' }}>
          ...
          <button 
            ...
            onClick={(e) => { e.stopPropagation(); setSelectedLeadDetail(lead); }}
            title="Edit"
          >
            <Pencil size={14} />
          </button>
      ...
      return null;
            ...
            <div className="lm-pagination">
              <span className="lm-pagination-info">
                ...
                {Math.min(page * PER_PAGE, filtered.length)} of {filtered.length} leads
              </span>
